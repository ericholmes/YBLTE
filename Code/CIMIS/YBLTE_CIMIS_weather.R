...
cimis_wy <- cimis_wy %>%
  transmute(
    ...
    temp = avg_air_temp_f,
    tempmax = max_air_temp_f,
    tempmin = min_air_temp_f
  ) %>%
  mutate(
    tule_fog = solar < 145 & wind < 6 & temp < 55 & precip <.01
  )
...
# Build a temp ribbon df --------------------------------------------------

temp_ribbon <- cimis_wy %>%
  transmute(
    date,
    ymin = tempmin,
    ymax = tempmax,
...
ggplot() +
  ...
  # temperature ribbon
  geom_ribbon(data = temp_ribbon,
              aes(x = date, ymin = ymin, ymax = ymax),
              fill = "brown", color = "grey50",alpha = 0.2, linewidth = .5) +
  ...
  scale_x_date(date_labels = "%b-1", breaks = "1 month", 
               limits = c(as.Date("2025-10-01"), as.Date("2026-05-01"))) +
