...
colusa_df <- data.frame(Y =   38.803888, X = -121.759886)
colusa_sf <- st_as_sf(colusa_df, coords = c("X", "Y"), crs = 4326)

colusa <- nhdplusTools::get_huc(AOI = colusa_sf, type = "huc08")

colusa_bbox <- st_bbox(colusa)
colusa_bbox_sf<- st_as_sfc(colusa_bbox)
...
#Load rivers
nhd_flow <- nhdplusTools::get_nhdplus(AOI = colusa, realization = "flowline")
...
#load lakes
nhd_wb <- nhdplusTools::get_waterbodies(AOI = colusa)
nhd_wb <- st_transform(nhd_wb, st_crs(cali_sf))

nhd_wb_centroids <- st_centroid(nhd_wb) #%>% st_coordinates()
nhd_wb_centroids[nhd_wb_centroids$areasqkm == 62.408,"gnis_name"] <- "Lake colusa"

# Download elevation data using the 'elevatr' package
elevation_data <- get_elev_raster(locations = colusa, z = 9, prj = st_crs(4326)$proj4string)
elevation_data <- mask(elevation_data, colusa)
...
##Save map
if(saveplot == T){jpeg("Output/Maps/Draped_hypso_dem_Colusa_inset_nhdfull.jpg",
...
map <- ggplot() +
  ...
  geom_sf(data = colusa, fill = NA, color = "grey40", linetype = 2) +
  ...
  coord_sf(xlim = c(colusa_bbox["xmin"]-.02, colusa_bbox["xmax"]+.02),
    ylim = c(colusa_bbox["ymin"]-.02, colusa_bbox["ymax"]+.02),
    expand = FALSE) +
...
inset <- ggplot() +
  ...
  geom_sf(data = colusa, fill = "grey20") +
  geom_sf(data = colusa_bbox_sf, fill = NA, color = "black") +
  theme_void()
