...
The Yolo Bypass Lower Trophic Expansion (YBLTE) builds on current lower trophic (LT) sampling in the bypass. LT includes phytoplankton and zooplankton that provide food for fish.
...
Dominating water sources: Sacramento River, Colusa Basin, Cache Creek, Putah Creek (not as influential this year)
...
### science in action!
```{r}
knitr::include_graphics("../docs/yblte_pic.png")
```
...
- Zoop score: qualitative assay of zooplankton density, exponential from 1-5
...
for(s in unique(dobodat_full$station)){
  dyD <- dygraph(dobodat_full %>% filter(station == s) 
...
make_all_station_plots <- function() {
  ...
  station_colors <- RColorBrewer::brewer.pal(n = length(stations), "Dark2")
  ...
  for (s in stations) {
    ...
    p_temp <- p_temp %>%
      add_trace(
        ...
        line = list(color = station_colors[s])
      ) %>% 
      add_trace(
        data = df_sb4,
        x = ~datetime, y = ~temp_c,
  ...
  subplot(p_temp, p_do, p_spc,
          ...
          shareX = TRUE,
          titleY = TRUE) %>%
  layout(xaxis = list(range = c(startdate, enddate)))
}
...
cyl_col <- c("#B63679FF",  "#440154FF",  "#482878FF", "#453581FF",  "#FB8861FF",
              "#34618DFF", "#2B748EFF",  "#24878EFF", "#1F998AFF", "#40BC72FF",
              "#67CC5CFF",  "#73A32F", "#CBE11EFF", "#FDE725FF")
cyl_col <- setNames(cyl_col, c("FWBN", "FW1", "KLWW","KNG3", "CCSYB",
